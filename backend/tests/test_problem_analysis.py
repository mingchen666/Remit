...
class ProblemAnalysisTests(unittest.IsolatedAsyncioTestCase):
    async def test_coordinator_expands_budget_after_truncated_json(self) -> None:
        valid_payload = {
            "title": "五问测试",
            "background": "验证截断恢复",
            "ques_count": 1,
            "ques1": "问题1：完成需求分析。",
            "analysis_summary": "先提取题意，再核验附件。",
            "question_analyses": {
                "ques1": {
                    "objective": "完成需求分析",
                    "input_data": ["附件1"],
                    ...
                    "dependencies": ["无"],
                    "risks": ["输出截断"],
                    "validation_requirements": ["结构完整性校验"],
                    "data_evidence": [],
                }
            },
        }
        ...
        agent = CoordinatorAgent("truncated-task", llm)

        result = await agent.run("问题1：完成需求分析。")

        self.assertEqual(result.questions["ques1"], "问题1：完成需求分析。")
        calls = llm.chat.await_args_list
        self.assertEqual(calls[0].kwargs["max_tokens"], 8192)
